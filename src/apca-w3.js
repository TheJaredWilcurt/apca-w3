 ...
 * GLOSSARY
 *
 * S-Luv - Is a Lˢ uˢvˢ-type colorspace for modeling vision and visual impairment perception of emissive displays and devices
 * SAPC - S-LUV Advanced Predictive Color
 * SACAM - S-Luv Accessible Color Appearance Model (Formerly known as SAPC)
 * APCA - Advanced Perceptual Contrast Algorithm
 * colorParsley - A separate color parsing library, used by this library
 *
 * Polarity:
 *   BoW - Black or dark text on White or light background
 *   WoB - White or light text on Black or dark background
 *
 * Color Term Abreviations:
 *   Y - Linear Y luminance. Luma Y′ (with prime) is distinguished from luminance Y, meaning that light intensity is nonlinearly encoded based on gamma corrected RGB primaries.
 *   U - One of the two chroma components
 *   V - The other chroma component
 *   Lc - Light Contrast
 *   Cb - Blue difference
 *   Cr - Red difference
 *
 * Colorspaces:
 *   sRGB - Most common color space used by computer monitors
 *   adobeRGB - Adobe's color space
 */
...
export function APCAcontrast (txtY, bgY, places = -1) {
  ...
  if (
    ...
    return 0.0;
    // optional string return for error
    // return 'error';
  };
  ...
  // For raw SAPC values
  let SAPC = 0.0;
  // For weighted final values
  let outputContrast = 0.0;
  // Alternate Polarity Indicator. N normal R reverse
  let polCat = 'BoW'; // Black text on White background
  ...
  // For normal polarity, black text on white background (BoW)
  if (bgY > txtY) {
...
export function invertAPCA ({
  ...
  unknownY = (unknownY > SA98G.blkThrs)
    ? unknownY
    : (Math.pow(((unknownY + SA98G.mOffsetIn)m* SA98G.mFactor), SA98G.mExp) * SA98G.mFactInv) - SA98G.mOffsetOut;
...
export function reverseAPCA (contrast = 0, knownY = 1.0, knownType = 'bg', returnAs = 'hex') {
  ...
  if (knownType == 'bg' || knownType == 'background') {
    ...
    if (isNaN(unknownY)) {
      ...
      unknownY = Math.pow(
        contrast + Math.pow(knownY, knownExp),
        1 / unknownExp
      );
      if (isNaN(unknownY)) {
        return false;
  ...
  // unclamp
  unknownY = (unknownY > SA98G.blkThrs) ? unknownY : (Math.pow(((unknownY + SA98G.mOffsetIn) * SA98G.mFactor), SA98G.mExp) * SA98G.mFactInv) - SA98G.mOffsetOut;
  ...
  if (returnAs === 'hex') {
    let hexB = (
      Math.round(Math.pow(unknownY, SA98G.mainTRCencode) * 255)
    ).toString(16).padStart(2, '0');
    return  '#' + hexB + hexB + hexB;
...
export function sRGBtoY (rgb = [0, 0, 0]) {
  ...
  /////   APCA   0.0.98G - 4g - W3 Compatible Constants   ////////////////////
  /*
  // 2.4 exponent emulates actual monitor perception
  const mainTRC = 2.4;
...
export function adobeRGBtoY (rgb = [0, 0, 0]) {
  ...
  // 2.35 exponent emulates actual monitor perception
  // Pending evaluation...
  const mainTRC = 2.35;
